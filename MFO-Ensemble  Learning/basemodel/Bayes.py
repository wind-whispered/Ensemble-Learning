...
metrics_filename = '../result/Bayes/model_metrics.xlsx'
...
plt.title('Confusion Matrix - NB', fontsize=16)
cm_filename = '../result/Bayes/confusion_matrix.pdf'
...
plt.title('ROC Curve - NB', fontsize=16)
plt.legend(loc="lower right", fontsize=12)
plt.grid(alpha=0.3)
roc_filename = './result/Bayes/roc_curves.pdf'
...
# 计算基线准确率
baseline_score = nb_model.score(X_test, y_test)
...
for feature in tqdm(feature_columns, desc="Calculating Feature Importances"):
    ...
    for _ in range(n_repeats):
        ...
        # 计算置换后的准确率
        permuted_score = nb_model.score(X_test_permuted, y_test)
        # 计算重要性（基线准确率 - 置换后准确率）
...
importance_filename = '../result/Bayes/feature_importance.xlsx'
...
importance_plot_filename = '../result/Bayes/feature_importance.pdf'
