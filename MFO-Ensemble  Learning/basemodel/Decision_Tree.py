...
metrics_filename = 'result/Decision_Tree/model_metrics.xlsx'
...
# cm_filename = '../result/决策树/confusion_matrix.pdf'
...
cm_filename = './result/Decision_Tree/confusion_matrix.pdf'
plt.savefig(cm_filename, format='pdf')
plt.close()
print(f"Confusion matrix saved to {cm_filename}")
...
plt.title('ROC Curve - DT', fontsize=16)
plt.legend(loc="lower right", fontsize=12)
plt.grid(alpha=0.3)
roc_filename = './result/Decision_Tree/roc_curves.pdf'
...
# 计算基线准确率
baseline_score = tree_model.score(X_test, y_test)
...
for feature in tqdm(feature_columns, desc="Calculating Feature Importances"):
    ...
    for _ in range(n_repeats):
        ...
        # 计算置换后的准确率
        permuted_score = tree_model.score(X_test_permuted, y_test)
        # 计算重要性（基线准确率 - 置换后准确率）
...
importance_filename = 'result/Decision_Tree/feature_importance.xlsx'
...
importance_plot_filename = 'result/Decision_Tree/feature_importance.pdf'
