...
metrics_filename = 'result/SVM/model_metrics.xlsx'
...
plt.tight_layout()
cm_filename = './result/SVM/confusion_matrix.pdf'
...
plt.title('ROC Curve - SVM', fontsize=30)
plt.legend(loc="lower right", fontsize=30)
plt.grid(alpha=0.3)
roc_filename = './result/SVM/roc_curve.pdf'
...
# 计算基线准确率
baseline_score = svm_model.score(X_test, y_test)
...
for feature in tqdm(feature_columns, desc="Calculating Feature Importances"):
    ...
    for _ in range(n_repeats):
        ...
        # 计算置换后的准确率
        permuted_score = svm_model.score(X_test_permuted, y_test)
        # 计算重要性（基线准确率 - 置换后准确率）
...
importance_filename = 'result/SVM/feature_importance.xlsx'
...
importance_plot_filename = 'result/SVM/feature_importance.pdf'
